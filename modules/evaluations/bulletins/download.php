<?php
/**
 * Module d'évaluations - Téléchargement de bulletin
...
requireLogin();
requirePagePermissionFromDB('evaluations', 'bulletins/download', 'read', '../../../dashboard.php');
...
if (!$eleve_id || !$periode) {
    showMessage('error', 'Paramètres manquants pour le téléchargement.');
    redirectTo('index.php');
}
...
switch ($format) {
    ...
    case 'pdf':
        // Mode PDF - pour l'instant, on génère du HTML optimisé
        // Dans une version future, on pourrait intégrer une librairie PDF comme TCPDF ou mPDF
        
        $filename = 'bulletin_' . $eleve['numero_matricule'] . '_' . $periode . '_' . date('Y-m-d') . '.html';
        ...
        // Inclure le template avec style pour PDF
        $format = 'pdf';
        include 'bulletin_template.php';
        break;
    ...
    default:
        // Format non supporté
        showMessage('error', 'Format de téléchargement non supporté.');
        redirectTo('individual.php?eleve=' . $eleve_id . '&periode=' . $periode);
}
?>
