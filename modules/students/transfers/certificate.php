...
// R̩cup̩rer les informations du transfert
...
// V̩rifier que le transfert est compl̩t̩
if ($transfer['statut'] !== 'complete') {
    showMessage('error', 'Le transfert doit ̻tre compl̩t̩ pour afficher le certificat');
    redirectTo("view-transfer.php?id=$transfer_id");
}

// G̩n̩rer un num̩ro de certificat s'il n'existe pas
if (!$transfer['numero_certificat']) {
    ...
    $database->query(
        "INSERT INTO transfer_history (transfer_id, action, ancien_statut, nouveau_statut, commentaire, user_id) VALUES (?, 'modification', ?, ?, 'Certificat g̩n̩r̩ automatiquement', ?)",
        [$transfer_id, $transfer['statut'], $transfer['statut'], $_SESSION['user_id']]
    );
    
    // Logger l'action
    logUserAction('generate_certificate', 'transfers', "Certificat g̩n̩r̩ automatiquement pour le transfert ID: $transfer_id", $transfer_id);
}

// Fonction de g̩n̩ration du certificat HTML
function generateCertificateHTML($transfer) {
    ...
    $html .= "<p style='font-size: 1.2rem; margin-top: 1.5cm; text-align: center; font-style: italic;'>
        ...
        <script>
            // Auto-focus sur la page pour une meilleure exp̩rience d'impression
            window.onload = function() {
                // Ajouter un d̩lai pour permettre le chargement complet
                setTimeout(function() {
                    window.focus();
                }, 500);
            };
